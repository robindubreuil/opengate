...
The DIY Friendly type B will, primarily to make soldering easier.
OpenGate type A and D are designed specifically to support the micro SD card format, which is now the industry standard for removable storage. Fullsize SD cards, while still in use in some devices, are becoming a deprecated format with no true advantage over micro SD. By focusing on micro SD support, we can ensure that OpenGate remains plug and play with the latest and most popular SD cards on the market, and provide the best possible user experience.
